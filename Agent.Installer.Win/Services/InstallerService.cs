...
using Nexus.Agent.Installer.Win.Utilities;
...
namespace Nexus.Agent.Installer.Win.Services
{
    public class InstallerService
    {
        ...
        private string InstallPath => Path.Combine(Path.GetPathRoot(Environment.SystemDirectory), "Program Files", "Nexus");
            ...
            bool createSupportShortcut)
        {
            try
            {
                ...
                await DownloadNexusAgent(serverUrl);

                FileIO.WriteAllText(Path.Combine(InstallPath, "ConnectionInfo.json"), Serializer.Serialize(connectionInfo));

                FileIO.Copy(Assembly.GetExecutingAssembly().Location, Path.Combine(InstallPath, "Nexus_Installer.exe"));
        ...
        public async Task<bool> Uninstall()
        {
            try
            {
                ...
                ProcessEx.StartHidden("cmd.exe", "/c sc delete Nexus_Service").WaitForExit();
                ...
                ProcessEx.StartHidden("netsh", "advfirewall firewall delete rule name=\"Nexus Desktop Unattended\"").WaitForExit();

                GetRegistryBaseKey().DeleteSubKeyTree(@"SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\Nexus", false);
        ...
        {
            var desktopExePath = Path.Combine(InstallPath, "Desktop", "Nexus_Desktop.exe");
            ProcessEx.StartHidden("netsh", "advfirewall firewall delete rule name=\"Nexus Desktop Unattended\"").WaitForExit();
            ProcessEx.StartHidden("netsh", $"advfirewall firewall add rule name=\"Nexus Desktop Unattended\" program=\"{desktopExePath}\" protocol=any dir=in enable=yes action=allow description=\"The agent that allows screen sharing and remote control for Nexus.\"").WaitForExit();
        }
        ...
        private void BackupDirectory()
        {
            if (Directory.Exists(InstallPath))
            {
                ...
                var backupPath = Path.Combine(Path.GetTempPath(), "Nexus_Backup.zip");
        ...
        private void CreateSupportShortcut(string serverUrl, string deviceUuid, bool createSupportShortcut)
        {
            ...
            shortcut.IconLocation = Path.Combine(InstallPath, "Nexus_Agent.exe");
        ...
        {
            var version = FileVersionInfo.GetVersionInfo(Path.Combine(InstallPath, "Nexus_Agent.exe"));
            var baseKey = GetRegistryBaseKey();

            var nexusKey = baseKey.CreateSubKey(@"SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\Nexus", true);
            nexusKey.SetValue("DisplayIcon", Path.Combine(InstallPath, "Nexus_Agent.exe"));
            nexusKey.SetValue("DisplayName", "Nexus");
            nexusKey.SetValue("DisplayVersion", version.FileVersion);
            nexusKey.SetValue("InstallDate", DateTime.Now.ToShortDateString());
            nexusKey.SetValue("Publisher", "nex-IT Jakub Potoczny");
            nexusKey.SetValue("VersionMajor", version.FileMajorPart.ToString(), RegistryValueKind.DWord);
            nexusKey.SetValue("VersionMinor", version.FileMinorPart.ToString(), RegistryValueKind.DWord);
            nexusKey.SetValue("UninstallString", Path.Combine(InstallPath, "Nexus_Installer.exe -uninstall -quiet"));
            nexusKey.SetValue("QuietUninstallString", Path.Combine(InstallPath, "Nexus_Installer.exe -uninstall -quiet"));
        }

        private async Task DownloadNexusAgent(string serverUrl)
        {
            var targetFile = Path.Combine(Path.GetTempPath(), $"Nexus-Agent.zip");
            ...
            {
                ProgressMessageChanged.Invoke(this, "Downloading Nexus agent.");
                using (var client = new WebClient())
                {
                    ...
                    await client.DownloadFileTaskAsync($"{serverUrl}/Content/Nexus-Win10-{Platform}.zip", targetFile);
                }
            }

            ProgressMessageChanged.Invoke(this, "Extracting Nexus files.");
            ...
            var wr = WebRequest.CreateHttp($"{serverUrl}/Content/Nexus-Win10-{Platform}.zip");
            wr.Method = "Head";
        ...
        private void InstallService()
        {
            ...
            ProgressMessageChanged?.Invoke(this, "Installing Nexus service.");
            var serv = ServiceController.GetServices().FirstOrDefault(ser => ser.ServiceName == "Nexus_Service");
            if (serv == null)
            {
                var command = new string[] { "/assemblypath=" + Path.Combine(InstallPath, "Nexus_Agent.exe") };
                var context = new InstallContext("", command);
                var serviceInstaller = new ServiceInstaller()
                {
                    Context = context,
                    DisplayName = "Nexus Service",
                    Description = "Background service that maintains a connection to the Nexus server.  The service is used for remote support and maintenance by this computer's administrators.",
                    ServiceName = "Nexus_Service",
                ...
                serv = ServiceController.GetServices().FirstOrDefault(ser => ser.ServiceName == "Nexus_Service");

                ProcessEx.StartHidden("cmd.exe", "/c sc.exe failure \"Nexus_Service\" reset= 5 actions= restart/5000");
            }
        ...
        {
            ProgressMessageChanged?.Invoke(this, "Stopping Nexus processes.");
            var procs = Process.GetProcessesByName("Nexus_Agent").Concat(Process.GetProcessesByName("Nexus_Desktop"));
        ...
        private void StopService()
        {
            try
            {
                var nexusService = ServiceController.GetServices().FirstOrDefault(x => x.ServiceName == "Nexus_Service");
                if (nexusService != null)
                {
                    Logger.Write("Stopping existing Nexus service.");
                    ProgressMessageChanged?.Invoke(this, "Stopping existing Nexus service.");
                    nexusService.Stop();
                    nexusService.WaitForStatus(ServiceControllerStatus.Stopped);
                }
            }
            catch (Exception ex)
            {
