    ...
    public class UpdaterWin : IUpdater
    {
        ...
        public async Task CheckForUpdates()
        {
            ...
            try
            {
                ...
                var fileUrl = serverUrl + $"/Content/Nexus-Win10-{platform}.zip";
        ...
        public async Task InstallLatestVersion()
        {
            try
            {
                ...
                var zipPath = Path.Combine(Path.GetTempPath(), "NexusUpdate.zip");

                var installerPath = Path.Combine(Path.GetTempPath(), "Nexus_Installer.exe");
                var platform = Environment.Is64BitOperatingSystem ? "x64" : "x86";

                await _updateDownloader.DownloadFile(
                     $"{serverUrl}/Content/Nexus_Installer.exe",
                     installerPath);
                ...
                foreach (var proc in Process.GetProcessesByName("Nexus_Installer"))
                {
                    proc.Kill();
                }
