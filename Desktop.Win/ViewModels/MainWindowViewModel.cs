...
using Nexus.Desktop.Win.Services;
using Nexus.Desktop.Win.Views;
using Nexus.Shared.Models;
using Nexus.Shared.Utilities;
using Nexus.Shared.Win32;
using System;
    ...
    public class MainWindowViewModel : BrandedViewModelBase
    {
            ...
            get
            {
                return new Executor((param) =>
                {
                    try
                    {
                        ...
                        psi.Arguments = $"/c sc create Nexus_Temp binPath=\"{filePath} {arguments} -elevate\"";
                        Process.Start(psi).WaitForExit();
                        psi.Arguments = "/c sc start Nexus_Temp";
                        Process.Start(psi).WaitForExit();
                        psi.Arguments = "/c sc delete Nexus_Temp";
                        Process.Start(psi).WaitForExit();
        ...
        public void PromptForHostName()
        {
                ...
                (serverUri.Scheme != Uri.UriSchemeHttp && serverUri.Scheme != Uri.UriSchemeHttps))
            {
                ...
                MessageBox.Show("Server URL must be a valid Uri (e.g. https://remote.nex-it.pl).", "Invalid Server URL", MessageBoxButton.OK, MessageBoxImage.Warning);
                return;
            }
