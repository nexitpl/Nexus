...
using Nexus.Desktop.XPlat.Services;
...
namespace Nexus.Desktop.XPlat.ViewModels
{
    public class FileTransferWindowViewModel : BrandedViewModelBase
    {
        ...
        public ICommand OpenFileUploadDialog => new Executor(async (param) =>
        {
            ...
            {
                initialDir = Directory.CreateDirectory(Path.Combine(Path.GetTempPath(), "Nexus_Shared")).FullName;
            }

            var ofd = new OpenFileDialog
            {
                Title = "Upload File via Nexus",
                AllowMultiple = true,
