...
ImmyBot experienced explosive growth this year and we only have 4 developers which is why there hasn't been much activity here. We did a big push earlier this year to build an integration between Nexus and ImmyBot but ran into some technical challenges trying to keep only one agent on machines, but we don't want to cannibalize Nexus for the gain of ImmyBot. We want to keep Nexus fully operable as a standalone project. When we resume this initiate later this year, you will likely abstraction of the Nexus.Desktop code into a shared library that can be consumed by Nexus as well as other agents like ImmyBot.


# Roadmap

* Complete Nexus <-> ImmyBot Integration (2022)
* Offer a cloud version of Nexus for individuals that do not wish to host it themselves (2023)

## Project Links
Subreddit: https://www.reddit.com/r/nexus_app/  
Docker: https://hub.docker.com/r/translucency/nexus  
Video Tutorials: https://nexus.one/Tutorials  

![image](.github/media/ask-remote.png)

## Disclaimer
Hosting a Nexus server requires running an ASP.NET Core web app behind IIS (Windows), Nginx (Ubuntu), or Caddy Server (any OS).  It's expected that the person deploying and maintaining the server is familiar with this process.  Since this is a hobby project that I develop in between working full time and raising a family, there simply isn't time available to provide support in this capacity.

## GitHub Actions
GitHub Actions allows you to build and deploy Nexus for free from their cloud servers.  Since the Windows agent can only be built on Windows, and the Mac agent can only be built on Mac, using a build platform like GitHub Actions or Azure Pipelines is the only reasonable way to build the whole project.  The definitions for the build processes are located in `/.github/workflows/` folder.
...
- By default, SQLite is used for the database.
    - The "Nexus.db" database file is automatically created in the root folder of your site.
...
* For Linux, you'll need to restart the Nexus service in systemd after overwriting the files.
...
The following steps will configure your Windows 10 machine for building the Nexus server and clients.
...
* KnownProxies: If your reverse proxy is on a different machine and is forwarding requests to the Nexus server, you will need to add the IP of the reverse proxy server to this array.
...
By default, Nexus uses a SQLite database.  When first run, it creates a file as specified for the SQLite connection string in appsettings.json.
...
## Logging
* On clients, logs are kept in %temp%\Nexus_Logs.log.
	* For the Agent running as a Windows service, this maps to C:\Windows\Temp\Nexus_Logs.log.
...
A shortcut to this page is placed in the `\Program Files\Nexus\` folder.  You can copy it anywhere you like.  You can also have it copied to the desktop automatically by using the `-supportshortcut` switch on the installer.
...
## Port Configuration
You can change the local port that the Nexus .NET server listens on by adding the below to `appsettings.Production.json`:
...
Alternatively, you can use a command-line argument for the `Nexus_Server` process or set an environment variable.
  - `--urls http://localhost:{port-number}`
  - `ASPNETCORE_URLS=http://localhost:{port-number}`

## API and Integrations
Nexus has a basic API, which can be browsed at https://nexus.lucency.co/swagger (or your own server instance).  Most endpoints require authentication via an API access token, which can be created by going to Account - API Access.

When accessing the API from the browser on another website, you'll need to set up CORS in appsettings by adding the website origin URL to the TrustedCorsOrigins array.  If you're not familiar with how CORS works, I recommend reading up on it before proceeding.  For example, if I wanted to create a login form on https://lucency.co that logged into the Nexus API, I'd need to add "https://lucency.co" to the TrustedCorsOrigins.
...
## Alerts
The Alerts API gives you the ability to add monitoring and alerting functionality to your device endpoints.  This feature is intended to add basic RMM-type functionality without diverging too far from Nexus's primary purpose.

Alerts can be set up to show a notification on the Nexus website, send an email, and/or perform a separate API request.
