<#
.SYNOPSIS
   Configures IIS and installs the Nexus server.
.COPYRIGHT
   Copyright ©  2020 nex-IT Jakub Potoczny.  All rights reserved.
#>
param (
    # The host name (excluding scheme) for the server that will run Nexus.
    [Parameter(Mandatory=$True)]
	[string]$HostName,
     # The name to use for the IIS Application Pool for the Nexus site.
    [Parameter(Mandatory=$True)]
	[string]$AppPoolName,
     # The name to use for the IIS site.
    [Parameter(Mandatory=$True)]
	[string]$SiteName,
    # The folder path where the Nexus server files are located.
    [Parameter(Mandatory=$True)]
    [string]$SitePath,
...
$Host.UI.RawUI.WindowTitle = "Nexus Setup"
Clear-Host
...
if ($OS.Name.ToLower().Contains("home") -or $OS.Caption.ToLower().Contains("home")) {
    Wrap-Host
    Wrap-Host "Error: Windows Home version does not have the necessary features to run Nexus." -ForegroundColor Red
    Do-Pause
    return
}
...
Wrap-Host
Wrap-Host "**********************************"
Wrap-Host "         Nexus Setup" -ForegroundColor Cyan
Wrap-Host "**********************************"
Wrap-Host
Wrap-Host "Hello, and thank you for trying out Nexus!" -ForegroundColor Green
Wrap-Host
Wrap-Host "This setup script will create an IIS site and install Nexus on this machine." -ForegroundColor Green
Wrap-Host
Do-Pause
Clear-Host
