...
using Nexus.Server.Components;
using Nexus.Server.Components.ModalContents;
...
namespace Nexus.Server.Pages
{
    public partial class ManageOrganization : AuthComponentBase
    {
        ...
        private readonly List<NexusUser> _orgUsers = new();
        ...
        [Inject]
        private UserManager<NexusUser> UserManager { get; set; }
        ...
        private async Task DeleteUser(NexusUser user)
        {
            if (!User.IsAdministrator)
            {
                return;
        ...
        private async Task EditDeviceGroups(NexusUser user)
        {
        ...
        private async Task ResetPassword(NexusUser user)
        {
            if (!User.IsAdministrator)
            {
                return;
        ...
        private async Task SendInvite()
        {
            ...
            else
            {
                ...
                var emailResult = await EmailSender.SendEmailAsync(invite.InvitedUser, "Invitation to Organization in Nexus",
                        $@"<img src='{NavManager.BaseUri}images/Nexus_Logo.png'/>
                            <br><br>
                            Hello!
                            <br><br>
                            You've been invited to join an organization in Nexus.
                            <br><br>
        ...
        private void SetUserIsAdmin(ChangeEventArgs args, NexusUser orgUser)
        {
            if (!User.IsAdministrator)
            {
                return;
