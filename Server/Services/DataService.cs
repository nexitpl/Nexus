...
namespace Nexus.Server.Services
{
    ...
    public interface IDataService
    {
        ...
        bool DoesUserHaveAccessToDevice(string deviceID, NexusUser nexusUser);

        bool DoesUserHaveAccessToDevice(string deviceID, string nexusUserID);

        string[] FilterDeviceIDsByUserPermission(string[] deviceIDs, NexusUser nexusUser);
        ...
        NexusUser[] GetAllUsersForServer();

        Task<NexusUser[]> GetAllUsersInOrganization(string orgId);
        ...
        int GetDeviceCount(NexusUser user);
        ...
        Task<NexusUser> GetUserAsync(string username);

        NexusUser GetUserByID(string userID);

        NexusUser GetUserByNameWithOrg(string userName);

        NexusUserOptions GetUserOptions(string userName);
        ...
        Task SetDisplayName(NexusUser user, string displayName);
        ...
        void UpdateUserOptions(string userName, NexusUserOptions options);
    ...
    public class DataService : IDataService
    {
        ...
        public async Task<bool> CreateUser(string userEmail, bool isAdmin, string organizationID)
        {
            ...
            try
            {
                var user = new NexusUser()
                {
                    ...
                    OrganizationID = organizationID,
                    UserOptions = new NexusUserOptions()
                };
                var org = dbContext.Organizations
                    .Include(x => x.NexusUsers)
                    .FirstOrDefault(x => x.ID == organizationID);
                dbContext.Users.Add(user);
                org.NexusUsers.Add(user);
                await dbContext.SaveChangesAsync();
                return true;
            }
            catch (Exception ex)
        ...
        public bool DoesUserHaveAccessToDevice(string deviceID, NexusUser nexusUser)
        {
            if (nexusUser is null)
            {
                return false;
            }
            ...
            return dbContext.Devices
                ...
                .Any(device => device.OrganizationID == nexusUser.OrganizationID &&
                    device.ID == deviceID &&
                    (
                        nexusUser.IsAdministrator ||
                        string.IsNullOrWhiteSpace(device.DeviceGroupID) ||
                        device.DeviceGroup.Users.Any(user => user.Id == nexusUser.Id
                    )));
        }

        public bool DoesUserHaveAccessToDevice(string deviceID, string nexusUserID)
        {
            using var dbContext = _appDbFactory.GetContext();

            var nexusUser = dbContext.Users.Find(nexusUserID);

            return DoesUserHaveAccessToDevice(deviceID, nexusUser);
        }

        public string[] FilterDeviceIDsByUserPermission(string[] deviceIDs, NexusUser nexusUser)
        {
            using var dbContext = _appDbFactory.GetContext();
            ...
            return dbContext.Devices
                ...
                .ThenInclude(x => x.Users)
                .Where(device =>
                    device.OrganizationID == nexusUser.OrganizationID &&
        ...
        public NexusUser[] GetAllUsersForServer()
        {
            using var dbContext = _appDbFactory.GetContext();
        ...
        public async Task<NexusUser[]> GetAllUsersInOrganization(string orgId)
        {
            if (string.IsNullOrWhiteSpace(orgId))
            {
                return Array.Empty<NexusUser>();
            }
            ...
            return organization.NexusUsers.ToArray();
        }
        ...
        public int GetDeviceCount(NexusUser user)
        {
            using var dbContext = _appDbFactory.GetContext();
        ...
        public async Task<NexusUser> GetUserAsync(string username)
        {
            if (string.IsNullOrWhiteSpace(username))
            {
                return null;
        ...
        public NexusUser GetUserByID(string userID)
        {
        ...
        public NexusUser GetUserByNameWithOrg(string userName)
        {
            if (userName == null)
            {
                return null;
        ...
        public NexusUserOptions GetUserOptions(string userName)
        {
            using var dbContext = _appDbFactory.GetContext();
        ...
        public bool JoinViaInvitation(string userName, string inviteID)
        {
            ...
            organization.NexusUsers.Add(user);
        ...
        public async Task SetDisplayName(NexusUser user, string displayName)
        {
            using var dbContext = _appDbFactory.GetContext();
        ...
        public void UpdateUserOptions(string userName, NexusUserOptions options)
        {
            using var dbContext = _appDbFactory.GetContext();
        ...
        public bool ValidateApiKey(string keyId, string apiSecret, string requestPath, string remoteIP)
        {
            ...
            var hasher = new PasswordHasher<NexusUser>();
