...
if ($SignAssemblies) {
    &"$Root\Utilities\signtool.exe" sign /f "$CertificatePath" /p $CertificatePassword /t http://timestamp.digicert.com "$Root\Server\wwwroot\Content\Win-x64\Nexus_Desktop.exe"
}
...
if ($SignAssemblies) {
    &"$Root\Utilities\signtool.exe" sign /f "$CertificatePath" /p $CertificatePassword /t http://timestamp.digicert.com "$Root\Server\wwwroot\Content\Win-x86\Nexus_Desktop.exe"
}
...
Copy-Item -Path "$Root\Agent.Installer.Win\bin\Release\Nexus_Installer.exe" -Destination "$Root\Server\wwwroot\Content\Nexus_Installer.exe" -Force
if ($SignAssemblies) {
    &"$Root\Utilities\signtool.exe" sign /f "$CertificatePath" /p $CertificatePassword /t http://timestamp.digicert.com "$Root\Server\wwwroot\Content\Nexus_Installer.exe"
}

# Compress Core clients.
$PublishDir =  "$Root\Agent\bin\Release\net6.0\win10-x64\publish"
Compress-Archive -Path "$PublishDir\*" -DestinationPath "$PublishDir\Nexus-Win10-x64.zip" -Force
while ((Test-Path -Path "$PublishDir\Nexus-Win10-x64.zip") -eq $false){
    Start-Sleep -Seconds 1
}
Move-Item -Path "$PublishDir\Nexus-Win10-x64.zip" -Destination "$Root\Server\wwwroot\Content\Nexus-Win10-x64.zip" -Force

$PublishDir =  "$Root\Agent\bin\Release\net6.0\win10-x86\publish"
Compress-Archive -Path "$PublishDir\*" -DestinationPath "$PublishDir\Nexus-Win10-x86.zip" -Force
while ((Test-Path -Path "$PublishDir\Nexus-Win10-x86.zip") -eq $false){
    Start-Sleep -Seconds 1
}
Move-Item -Path "$PublishDir\Nexus-Win10-x86.zip" -Destination "$Root\Server\wwwroot\Content\Nexus-Win10-x86.zip" -Force

$PublishDir =  "$Root\Agent\bin\Release\net6.0\linux-x64\publish"
Compress-Archive -Path "$PublishDir\*" -DestinationPath "$PublishDir\Nexus-Linux.zip" -Force
while ((Test-Path -Path "$PublishDir\Nexus-Linux.zip") -eq $false){
    Start-Sleep -Seconds 1
}
Move-Item -Path "$PublishDir\Nexus-Linux.zip" -Destination "$Root\Server\wwwroot\Content\Nexus-Linux.zip" -Force
